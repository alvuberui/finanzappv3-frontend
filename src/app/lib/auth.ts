...
async function refreshAccessToken(token: Token): Promise<Token> {
  try {
    ...
    if (!res.ok) {
      ...
      // ✅ evita reintentos infinitos con refresh token inválido
      return {
        ...token,
...
export const {
  ...
  callbacks: {
    async jwt({ token, account, profile }: any) {
      ...
      // Login inicial / refresh por provider
      ...
      // Si aún es válido (margen 60s), no hacemos nada
      if (t.expiresAt && Date.now() < t.expiresAt - 60_000) return t

      // ✅ Si ya falló antes, NO reintentar (evita spam de logs)
