...
type EditMovementPayload = {
  movementType: MovementType;
  amount: number;
  date: string;
  tagId?: number | null;   // ✅ aceptar null explícitamente
  description?: string;
};
...
export default function EditMovementPage() {
  ...
  const [movement, setMovement] = useState<MovementFromBff | null>(null);

  const [serverMsg, setServerMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);

  // ✅ tags desde backend (igual que create)
  ...
  useEffect(() => {
    ...
    async function loadMovement() {
      ...
      try {
        ...
        const res = await fetch(`/api/movements/${id}`, { method: "GET", cache: "no-store" });
        const json = await res.json().catch(() => null);

        if (!res.ok || json?.ok === false) {
          throw new Error(json?.error?.message ?? "Error cargando el movimiento");
        }

        if (!cancelled) {
          setMovement(json.data as MovementFromBff);
          setLoading(false);
        }
      } catch (e: any) {
        if (!cancelled) {
          setLoadErr(e?.message ?? "Error cargando el movimiento");
          setLoading(false);
        }
      }
    }
  ...
  const initialValues = useMemo(
    () => ({
      movementType: (movement?.movementType ?? "ESSENTIAL_EXPENSE") as MovementType,
      amount: movement ? String(movement.amount) : "",
      date: movement?.date ?? todayISO(),

      // ✅ ahora es select: string
      tagId: movement?.tagId ? String(movement.tagId) : "",
  ...
  function selectedTag(values: { tagId: string }) {
    const tid = toOptionalPositiveInt(values.tagId);
    if (!tid) return null;
    return tags.find((t) => t.id === tid) ?? null;
  }
  ...
  const payload: EditMovementPayload = {
    movementType: values.movementType,
    amount: toNumber(values.amount),
    date: values.date,
    tagId: tagIdParsed ?? null, // <-- siempre number o null
  ...
  if (!res.ok || json?.ok === false) {
    throw new Error(json?.error?.message ?? "Error editando el movimiento");
  }
    ...
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-gray-100 flex items-center justify-center px-6 py-14">
      <div className="w-full max-w-3xl">
        <header className="text-center mb-10">
          ...
          <p className="mt-3 text-gray-300">Modifica los datos del movimiento.</p>
        </header>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl shadow-lg p-6 md:p-8">
          {loading ? (
            ...
            <div className="rounded-2xl border border-amber-400/30 bg-amber-500/10 text-amber-100 px-4 py-3 text-sm">
              {loadErr}
            </div>
          ) : (
            <Formik
              ...
              onSubmit={async (values, helpers) => {
                try {
                  await submit(values);
                  setServerMsg({ type: "ok", text: "Movimiento actualizado correctamente." });
                  ...
                  setServerMsg({
                    type: "err",
                    text: e?.message ?? "Error actualizando el movimiento.",
                  });
                  helpers.setSubmitting(false);
                }
              }}
            >
              {({ isSubmitting, isValid, values }) => {
                const sel = selectedTag(values);
                const selColor = normalizeHexColor(sel?.color) ?? null;
                          ...
                          {sel ? (
                            <div className="mt-2 flex items-center gap-2 text-xs text-gray-300">
                              <span
                                ...
                                style={{ backgroundColor: selColor ?? "transparent" }}
                              />
                              <span>{sel.name}</span>
                            </div>
                          ) : null}
